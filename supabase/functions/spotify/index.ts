...
serve(async (req) => {
  ...
  try {
    ...
    const urlMatch = query.match(/(?:spotify\.com\/artist\/|spotify:artist:)([a-zA-Z0-9]+)/);
    
    if (urlMatch && urlMatch[1]) {
      artistId = urlMatch[1];
      console.log(`[Spotify] Extracted Artist ID from URL: ${artistId}`);
      searchQuery = `spotify:artist:${artistId}`;
    }
    ...
    if (!artist || artist.error) {
      return new Response(
        JSON.stringify({ error: "Artist not found on Spotify", details: artist || searchData }),
        { status: 404, headers: { ...corsHeaders, "Content-Type": "application/json" } }
      );
    }

    console.log(`[Spotify] Found: ${artist.name || artistId} (${artistId})`);
