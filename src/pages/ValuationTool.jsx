...
const suggestedArtists = ["Taylor Swift", "Drake", "The Weeknd", "Bad Bunny", "Ariana Grande"];
...
const ValuationTool = () => {
  ...
  useEffect(() => {
    const fetchSuggestions = async () => {
      ...
      if (!searchQuery.trim()) { setSuggestions([]); setShowSuggestionsDropdown(false); return; }
      setIsLoadingSuggestions(true);
      setShowSuggestionsDropdown(true);
      try {
        let results = suggestedArtists.filter((a) => a.toLowerCase().includes(searchQuery.toLowerCase()));
        if (searchQuery.length > 2) {
          try {
            const apiResults = await getArtistSuggestions(searchQuery, platform);
            results = [...new Set([...results, ...apiResults.map((r) => r.name)])];
          } catch {}
        }
    ...
    try {
      ...
      if (platform === "spotify") result = await searchApify(searchQuery);
        ...
        if (result.type === "channel_list") {
          setYoutubeChannels(result.channels); setShowChannelSelector(true);
  ...
  const handleSuggestionClick = async (artist) => {
    setSearchQuery(artist); setShowSuggestionsDropdown(false);
    setShouldShowSuggestions(false); setError(null);
    setIsLoading(true); setSelectedArtist(null);
    setShowChannelSelector(false); setYoutubeChannels([]);
    try {
      let result;
      ...
      <div className="max-w-7xl mx-auto space-y-6 sm:space-y-8">
        ...
        <div className="text-center space-y-3 sm:space-y-4">
          ...
          <p className="text-sm sm:text-lg text-slate-500 dark:text-slate-400 max-w-xl mx-auto">
            Discover insights &amp; metrics from Spotify and YouTube in real-time
          </p>
        </div>

        {/* ── Search Card ───────────────────────────────────── */}
        <div className={`relative overflow-hidden rounded-3xl bg-gradient-to-br ${cfg.color} shadow-2xl`}>
          {/* Decorative blobs */}
          ...
          <div className="relative z-10 p-5 sm:p-7 lg:p-10">
            ...
            <div className="flex items-center gap-3 sm:gap-4 mb-5 sm:mb-7">
              ...
              <div>
                <h2 className="text-xl sm:text-2xl lg:text-3xl font-black text-white tracking-tight">Search Artist</h2>
                <p className="text-white/70 text-xs sm:text-sm mt-0.5">
                  Discover analytics from <span className="font-semibold text-white/90">{cfg.label}</span>
                </p>
              </div>
            </div>
            ...
            <div className="flex gap-2 sm:gap-3 flex-col lg:flex-row">
              ...
              <div className="flex-1 relative">
                ...
                <input
                  ...
                  onFocus={() => { if (searchQuery.trim() && suggestions.length > 0) setShowSuggestionsDropdown(true); }}
                ...
                {searchQuery && (
                  <button
                    onClick={() => { setSearchQuery(""); setSuggestions([]); setShowSuggestionsDropdown(false); inputRef.current?.focus(); }}
                    ...
                    <div className="absolute top-full left-0 right-0 mt-2 bg-white dark:bg-slate-800 rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-700 overflow-hidden z-40 max-h-64 overflow-y-auto">
                      {isLoadingSuggestions ? (
                        ...
                        suggestions.map((suggestion, idx) => (
                          ...
                          >
                            <Search size={14} className="text-slate-400 group-hover:text-slate-600 dark:group-hover:text-slate-300 transition-colors flex-shrink-0" />
                            <span className="text-sm font-medium group-hover:text-slate-900 dark:group-hover:text-white transition-colors">{suggestion}</span>
                          </button>
                        ))
                      )}
                    </div>
            ...
            {isLoading && platform === "spotify" && (
              <div className="mt-4 flex items-center gap-2.5 p-4 bg-white/15 border border-white/25 rounded-xl backdrop-blur-xl">
                <Loader2 size={17} className="text-white animate-spin flex-shrink-0" />
                <p className="text-white text-sm font-semibold">Fetching Spotify data…</p>
              </div>
            )}
            ...
            {!isLoading && !error && (
              <div className="mt-4 flex items-start gap-2.5 p-4 bg-white/10 border border-white/20 rounded-xl backdrop-blur-xl">
                <Info size={15} className="text-white/70 flex-shrink-0 mt-0.5" />
            ...
            <div className="mt-5 sm:mt-7">
              <p className="text-xs font-bold text-white/70 uppercase tracking-widest mb-3">Try searching for:</p>
            ...
            <div className="flex flex-col items-center gap-5 sm:gap-6 max-w-xl mx-auto px-4">
              <div className="relative">
                ...
                <div className="relative p-5 sm:p-6 bg-gradient-to-br from-emerald-100 to-blue-100 dark:from-emerald-900/30 dark:to-blue-900/30 rounded-full">
                  <Search size={44} className="sm:w-14 sm:h-14 text-emerald-600 dark:text-emerald-400" />
                </div>
              </div>
              <div>
                ...
                <p className="text-sm sm:text-base text-slate-500 dark:text-slate-400 mb-5 sm:mb-6">
                  Select a platform and search for an artist to begin your analysis
                </p>
                <div className="space-y-2.5 text-sm text-slate-400 dark:text-slate-500">
                  ...
                  <p className="flex items-center justify-center gap-2">
                    <span className="text-xl">🎵</span>
                    Use <strong className="text-emerald-600 dark:text-emerald-400">Spotify</strong> for detailed stream counts
                  </p>
                </div>
              </div>
            </div>
