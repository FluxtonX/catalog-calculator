...
const PLATFORM_CONFIG = {
  ...
  youtube: {
    ...
    bgPattern: "radial-gradient(circle at 70% 50%, rgba(239, 68, 68, 0.25) 0%, transparent 60%)",
...
const PLATFORM_FEATURES = {
  ...
  itunes: ["30s Previews", "Album Catalog", "Popularity Score", "Apple Music Links"],
};
  ...
  return (
    <Select.Root value={platform} onValueChange={setPlatform} disabled={isLoading}>
      ...
      >
        ...
        <Select.Icon>
          <ChevronDown size={15} className="opacity-70 group-data-[state=open]:rotate-180 transition-transform duration-200" />
        </Select.Icon>
      </Select.Trigger>
                ...
                >
                  <div className="flex items-center gap-3">
                    <div className={`p-1.5 rounded-lg ${isSelected ? config.iconBg : "bg-slate-100 dark:bg-slate-700"} transition-colors`}>
                      <PIcon size={15} className={isSelected ? config.iconColor : "text-slate-500 dark:text-slate-400"} />
                    </div>
                    <div>
                      <Select.ItemText>{config.label}</Select.ItemText>
                      <p className="text-[10px] text-slate-400 dark:text-slate-500 font-normal mt-0.5">
                        {key === "spotify" ? "via Apify scraper" : key === "youtube" ? "via YouTube API" : "via iTunes API"}
                      </p>
                    </div>
                  </div>
      ...
      <div className="max-w-7xl mx-auto space-y-6 sm:space-y-8">
        ...
        <div className="text-center space-y-3 sm:space-y-4">
          ...
          <p className="text-sm sm:text-lg text-slate-500 dark:text-slate-400 max-w-xl mx-auto">
            Real-time valuation, pricing analysis, and buyer insights powered by Spotify, YouTube & Apple Music data
          </p>
                    ...
                    <div className="absolute top-full left-0 right-0 mt-2 bg-white dark:bg-slate-800 rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-700 overflow-hidden z-40 max-h-64 overflow-y-auto">
                      ...
                      {isLoadingSuggestions ? (
                        ...
                        suggestions.map((suggestion, idx) => (
                          ...
                          >
                            <div className={`p-1.5 rounded-lg ${cfg.iconBg || "bg-slate-100 dark:bg-slate-700"} flex-shrink-0`}>
                              <Search size={12} className={cfg.iconColor || "text-slate-500"} />
                            </div>
            ...
            {isLoading && (
              <div className="mt-4 flex items-center gap-3 p-4 bg-white/15 border border-white/25 rounded-xl backdrop-blur-xl">
                <div className="relative flex-shrink-0">
                  ...
                  <div className="absolute inset-0 w-8 h-8 rounded-full border-2 border-white/10 border-b-white/60 animate-spin" style={{ animationDuration: "1.5s", animationDirection: "reverse" }} />
                </div>
                <div>
                  <p className="text-white text-sm font-bold">Crunching the numbers…</p>
                  <p className="text-white/60 text-xs mt-0.5">Fetching live data from {cfg.label}</p>
                </div>
              </div>
            )}
        ...
        {isLoading && (
          <div className="space-y-4 animate-pulse">
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
              {[...Array(4)].map((_, i) => (
                <div key={i} className="h-28 bg-gradient-to-r from-slate-200 via-slate-100 to-slate-200 dark:from-slate-800 dark:via-slate-700 dark:to-slate-800 rounded-2xl" />
              ))}
            </div>
        ...
        {!isLoading && selectedArtist && (
          <div className="space-y-5 sm:space-y-6">
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-4">
              ...
              <div className={`flex items-center gap-2 px-3 py-1.5 ${cfg.liveBadgeBg} border rounded-full`}>
                <span className={`w-2 h-2 rounded-full animate-pulse ${cfg.liveDot}`} />
                ...
                <div className="grid grid-cols-3 gap-2 sm:gap-3 mb-6">
                  {Object.entries(PLATFORM_CONFIG).map(([key, config]) => {
                    ...
                    return (
                      ...
                      >
                        <PIcon size={20} className={isActive ? "text-white" : ""} />
