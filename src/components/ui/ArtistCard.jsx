...
const ArtistCard = ({
  ...
  return (
    <div className="space-y-4 sm:space-y-6 px-2 sm:px-0">
      ...
      <div className="rounded-3xl overflow-hidden border-2 border-emerald-200 dark:border-emerald-800/60 shadow-xl bg-gradient-to-br from-emerald-50 to-teal-50 dark:from-emerald-950/40 dark:to-teal-950/40">
        <button
      ...
      ${
        ...
        {showValuation && (
          <div className="border-t-2 border-emerald-200 dark:border-emerald-800/60 p-4 sm:p-6 lg:p-8 bg-white dark:bg-slate-900">
            {platform === "youtube" ? (
              <YouTubeValuationTab
                artistData={{
                  name,
          ...
          <div className="p-4 sm:p-6 lg:p-8 min-h-[380px] sm:min-h-[480px]">
            ...
            >
              {singles?.length > 0 ? (
                <MediaGrid>
                  {singles.map((s, i) => (
                    <SingleCard key={s.id || i} single={s} index={i} />
                  ))}
                </MediaGrid>
              ) : (
              ...
              {popularReleases?.length > 0 ? (
                <MediaGrid>
                  {popularReleases.map((r, i) => (
                    <PopularReleaseCard key={r.id || i} release={r} index={i} />
                  ))}
                </MediaGrid>
              ) : (
                <EmptyState
