...
// Logic
import {
  parseViewCount, calculateYouTubeMetrics,
  formatNumber, formatCurrency, CONTENT_ID_MULTIPLIER,
...
const YouTubeValuationTab = ({ artistData }) => {
  ...
  const metrics = calculateYouTubeMetrics({
    totalViews, annualViewPercentage, monetizationRate,
    avgCpm, creatorCut, streamingRate,
  });
  ...
  const handleSave = async () => {
    try {
      ...
      const reportData = {
        ...
        inputs: { totalViews, annualViewPercentage, monetizationRate, avgCpm, creatorCut, streamingRate, contentIdMultiplier: CONTENT_ID_MULTIPLIER },
        calculations: { ...metrics },
        valuations: { conservative: metrics.conservativeValuation, market: metrics.marketValuation, premium: metrics.premiumValuation, advancePackage: metrics.totalAdvancePackage, caccAdjusted: metrics.caccAdjustedValuation },
      };
      generateYouTubeValuationPDF(reportData);
      const { error: saveError } = await supabase.from("user_reports").insert([{ user_id: user.id, artist_name: artistData.name, report_type: "youtube_valuation", report_data: reportData }]).select().single();
  ...
  return (
    <div className="space-y-5 sm:space-y-6">
      ...
      <ValuationAssumptions
        annualViewPercentage={annualViewPercentage} setAnnualViewPercentage={setAnnualViewPercentage}
        monetizationRate={monetizationRate} setMonetizationRate={setMonetizationRate}
        avgCpm={avgCpm} setAvgCpm={setAvgCpm}
        creatorCut={creatorCut} setCreatorCut={setCreatorCut}
        streamingRate={streamingRate} setStreamingRate={setStreamingRate}
      />

      <AnnualRevenueCard totalAnnualRevenue={metrics.totalAnnualRevenue} formatCurrency={formatCurrency} />

      <RevenueAnalysis {...metrics} monetizationRate={monetizationRate} creatorCut={creatorCut} streamingRate={streamingRate} formatNumber={formatNumber} formatCurrency={formatCurrency} />

      <RevenueStreams adRevenue={metrics.adRevenue} streamingRevenue={metrics.streamingRevenue} totalAnnualRevenue={metrics.totalAnnualRevenue} formatCurrency={formatCurrency} />

      <ValuationScenarios {...metrics} totalAnnualRevenue={metrics.totalAnnualRevenue} formatCurrency={formatCurrency} />

      <YoutubeSaveButton hasValidData={hasValidData} isSaving={isSaving} onSave={handleSave} />
    </div>
  );
};
