      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 sm:gap-8">
        ...
        <div className="space-y-1">
          ...
          <div className="flex justify-between items-center py-2.5 bg-red-50 dark:bg-red-900/20 rounded-xl px-3 mt-2">
            ...
            <span className="text-xs sm:text-sm font-black text-red-600 dark:text-red-400">{formatCurrency(streamingRevenue)}</span>
          </div>
        </div>
        ...
        <div className="space-y-1">
          ...
          {[
            ...
            { label: "Streaming Revenue", value: formatCurrency(streamingRevenue), color: "text-red-600 dark:text-red-400" },
          ].map((row) => (
            <div key={row.label} className="flex justify-between items-center py-2.5 border-b border-slate-100 dark:border-slate-800">
              <span className="text-xs sm:text-sm text-slate-500 dark:text-slate-400">{row.label}</span>
          ...
          <div className="flex justify-between items-center py-2.5 bg-red-50 dark:bg-red-900/20 rounded-xl px-3 mt-2">
            ...
            <span className="text-xs sm:text-sm font-black text-red-600 dark:text-red-400">{formatCurrency(totalAnnualRevenue)}</span>
          </div>
        </div>
      </div>

      {/* Info note */}
      <div className="mt-6 flex items-start gap-2.5 p-4 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-500/30 rounded-xl">
        <Info size={15} className="text-red-500 mt-0.5 flex-shrink-0" />
