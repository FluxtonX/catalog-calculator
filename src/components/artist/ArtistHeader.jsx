...
import {
  Music, Users, TrendingUp, Disc3, Youtube,
  ExternalLink, Globe, Facebook, Instagram, Twitter, Headphones,
} from "lucide-react";
...
const SocialLink = ({ href, label, Icon }) => (
  <Tooltip.Provider delayDuration={150}>
    <Tooltip.Root>
      ...
      <Tooltip.Portal>
        <Tooltip.Content className="px-2.5 py-1.5 text-xs bg-slate-900 text-white rounded-lg shadow-xl z-50" sideOffset={6}>
          Open {label}
...
const ArtistHeader = ({
  ...
  onLaunchValuation, getSocialIcon,
}) => {
  ...
  const platformLabel = isItunes ? "Apple Music" : isYoutube ? "YouTube" : "Spotify";
          ...
          <div className="flex-1 w-full min-w-0">
            ...
            <div className="flex items-center gap-2 sm:gap-3 mb-3 sm:mb-4 flex-wrap">
              <div className={`flex items-center gap-1.5 px-3 py-1.5 border rounded-full ${liveRingColor}`}>
                ...
                <span className={`text-xs font-bold uppercase tracking-wide ${liveTextColor}`}>Live Data</span>
              </div>
              ...
              {isItunes && (
                <div className="flex items-center gap-1.5 px-2.5 py-1 bg-white/10 border border-white/15 rounded-full">
                  <span className="text-white/70 text-[10px] font-bold uppercase tracking-wide">Apple Music</span>
                </div>
              )}
            </div>
            ...
            <div className="flex flex-wrap items-center gap-2 sm:gap-3 mb-4 sm:mb-5">
              {followers && (
                <StatPill
                  ...
                  gradient={isItunes ? "from-slate-600 to-zinc-700" : "from-blue-500 to-blue-700"}
                />
              )}
              ...
              {popularity && !isApify && (
                <StatPill
                  ...
                  value={popularity}
                  label="Popularity"
                  gradient={isItunes ? "from-zinc-600 to-slate-700" : "from-purple-500 to-purple-700"}
                />
              )}
            </div>
                ...
                {genres.slice(0, 6).map((genre, i) => (
                  <span
                    ...
                    className={`px-2.5 py-1 sm:px-3 sm:py-1.5 bg-white/10 hover:bg-white/15 backdrop-blur-sm border rounded-full text-[10px] sm:text-xs font-semibold text-white/85 hover:text-white transition-all duration-200 cursor-default capitalize ${
                      isItunes ? "border-white/15 hover:border-white/30" : "border-white/15 hover:border-white/25"
                    }`}
                  >
                    {genre}
                  </span>
