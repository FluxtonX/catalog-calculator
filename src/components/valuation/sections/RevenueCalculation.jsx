...
const RevenueCalculation = ({
  ...
  const { royaltyShare = 100, currency = 'USD', exchangeRates = { USD: 1 } } = useArtistStore();
  const rate = exchangeRates[currency] || 1;

  // Build the math formula string
  let mathParts = [`${formatNumber(monthlyStreamsEst)} streams × $${effectiveSpotifyRate.toFixed(4)}`];
  if (royaltyShare < 100) {
    mathParts.push(`× ${royaltyShare}% share`);
  }
  if (currency !== 'USD') {
    mathParts.push(`× ${rate.toFixed(4)} ${currency}/USD`);
  }
  const mathSubtext = mathParts.join(" ");
  ...
  return (
    <div className="bg-white dark:bg-slate-900 rounded-3xl border-2 border-slate-200 dark:border-slate-800 shadow-xl p-4 sm:p-6 lg:p-8">
      ...
      <div className="space-y-3">
        ...
        />
        <CalcRow label="Spotify Payout Rate (Base USD)"
