...
const MethodologyNote = () => (
  <Accordion.Root type="single" collapsible className="bg-gradient-to-r from-emerald-50 to-slate-50 dark:from-emerald-900/10 dark:to-slate-900 rounded-3xl border-2 border-emerald-200 dark:border-emerald-500/20 shadow-xl overflow-hidden">
    <Accordion.Item value="methodology">
      <Accordion.Trigger className="w-full flex items-center gap-4 p-5 sm:p-7 text-left group">
        ...
        <div className="flex-1">
          <p className="font-bold text-emerald-800 dark:text-emerald-300 text-sm sm:text-base">Valuation Methodology</p>
          <p className="text-xs text-emerald-600 dark:text-emerald-400 mt-0.5">Click to view calculation details</p>
        </div>
        <ChevronDown size={18} className="text-emerald-500 transition-transform duration-300 group-data-[state=open]:rotate-180 flex-shrink-0" />
      </Accordion.Trigger>
      <Accordion.Content className="px-5 sm:px-7 pb-5 sm:pb-7 data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0">
        <ul className="text-xs sm:text-sm text-emerald-700 dark:text-emerald-400 space-y-2 list-disc list-inside border-t border-emerald-200 dark:border-emerald-500/20 pt-4">
